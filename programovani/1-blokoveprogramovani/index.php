<!DOCTYPE html>
<html lang='cs' data-bs-theme="dark">

<head>
    <title>DDM - Filip Komárek</title>
    <link rel="icon" type="image/x-icon" href="./favicon.ico">
    <meta charset='utf-8'>
</head>

<body>
    <?php include ("../../src/link.html"); ?>

    <div class="container">

        <a href="../"><button class="btn btn-primary" type="button">Zpět</button></a>

        <h1 class="nadpis">Bludiště</h1>

        <div class="alert alert-warning d-md-none">
            Upozornění: Tato stránka není optimalizována pro mobilní telefony a pro zařízení
            s menší obrazovkou. Některé prvky mohou být příliš malé nebo jinak nefunkční.
        </div>

        <p>
            V tomto kroužku budeme programovat v C#, nicméně je dobré začít blokovým programováním, které nás naučí základní logiku.
        </p>

        <p>
            Začneme jednoduchou hrou na stránce
            <a href="https://blockly.games/?lang=cs" target="_blank">blockly.games</a>.
            Zde je soubor úloh, které nám pomohou se začátkem. Níže budou řešení na ty úlohy.
        </p>

        <h2>Řešení úloh</h2>

        <h3>Skládačka</h3>
        <button class="btn btn-primary" data-bs-toggle="collapse" data-bs-target="#imageCollapse101">řešení</button>
        <div id="imageCollapse101" class="collapse mt-3">
            <img src="./101.png" alt="Řešení ještě nebylo uvolněno">
        </div>

        <h3>Bludiště</h3>
        <ol>
            <li>
                <button class="btn btn-primary" data-bs-toggle="collapse"
                    data-bs-target="#imageCollapse201">řešení</button>
                <div id="imageCollapse201" class="collapse mt-3">
                    <img src="./201.png" alt="Řešení ještě nebylo uvolněno">
                </div>
            </li>

            <li>
                <button class="btn btn-primary" data-bs-toggle="collapse"
                    data-bs-target="#imageCollapse202">řešení</button>
                <div id="imageCollapse202" class="collapse mt-3">
                    <img src="./202.png" alt="Řešení ještě nebylo uvolněno">
                </div>
            </li>

            <li>
                <button class="btn btn-primary" data-bs-toggle="collapse"
                    data-bs-target="#imageCollapse203">řešení</button>
                <div id="imageCollapse203" class="collapse mt-3">
                    <img src="./203.png" alt="Řešení ještě nebylo uvolněno">
                </div>
            </li>

            <li>
                <button class="btn btn-primary" data-bs-toggle="collapse"
                    data-bs-target="#imageCollapse204">řešení</button>
                <div id="imageCollapse204" class="collapse mt-3">
                    <img src="./204.png" alt="Řešení ještě nebylo uvolněno">
                </div>
            </li>

            <li>
                <button class="btn btn-primary" data-bs-toggle="collapse"
                    data-bs-target="#imageCollapse205">řešení</button>
                <div id="imageCollapse205" class="collapse mt-3">
                    <img src="./205.png" alt="Řešení ještě nebylo uvolněno">
                </div>
            </li>

            <li>
                <button class="btn btn-primary" data-bs-toggle="collapse"
                    data-bs-target="#imageCollapse206">řešení</button>
                <div id="imageCollapse206" class="collapse mt-3">
                    <img src="./206.png" alt="Řešení ještě nebylo uvolněno">
                </div>
            </li>

            <li>
                <button class="btn btn-primary" data-bs-toggle="collapse"
                    data-bs-target="#imageCollapse207">řešení</button>
                <div id="imageCollapse207" class="collapse mt-3">
                    <img src="./207.png" alt="Řešení ještě nebylo uvolněno">
                </div>
            </li>

            <li>
                <button class="btn btn-primary" data-bs-toggle="collapse"
                    data-bs-target="#imageCollapse208">řešení</button>
                <div id="imageCollapse208" class="collapse mt-3">
                    <img src="./208.png" alt="Řešení ještě nebylo uvolněno">
                </div>
            </li>

            <li>
                <button class="btn btn-primary" data-bs-toggle="collapse"
                    data-bs-target="#imageCollapse209">řešení</button>
                <div id="imageCollapse209" class="collapse mt-3">
                    <img src="./209.png" alt="Řešení ještě nebylo uvolněno">
                </div>
            </li>

            <li>
                <button class="btn btn-primary" data-bs-toggle="collapse"
                    data-bs-target="#imageCollapse210">řešení</button>
                <div id="imageCollapse210" class="collapse mt-3">
                    <p>pro tento bod jsou tu dvě řešení, jedno které je perfektní z hlediska cesty a druhé má nejméně
                        bloků
                        (druhé udělal Miky Honys)</p>
                    <img src="./210.png" alt="Řešení ještě nebylo uvolněno">
                    <p>nebo</p>
                    <img src="./210-2.png" alt="Řešení ještě nebylo uvolněno">
                </div>
            </li>
        </ol>

        <h2>Příliš snadné?</h2>
        <p>
            Zkuste si zatím na <a href="https://blockly.games/?lang=cs" target="_blank">blockly.games</a>
            dát další levely v dalších kategoriích, dokud ostatní nedokončí svoji práci.
        </p>

    </div>

</body>

</html>